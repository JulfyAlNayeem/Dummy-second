...
const ImagePreviewModal = ({ previewImage, setPreviewImage, conversationId, user }: any): JSX.Element | null => {
  ...
  useEffect(() => {
    if (previewImage && previewImages.length === 0) {
      setPreviewImages([{ id: 'initial', media: [{ url: previewImage.replace(`${MEDIA_BASE_URL}uploads/`, ''), type: 'image' }], createdAt: new Date() }]);
    }
  ...
  const handleNextImage = useCallback((e) => {
    ...
    if (currentImageIndex < previewImages.length - 1) {
      ...
      setPreviewImage(`${MEDIA_BASE_URL}uploads/${previewImages[currentImageIndex + 1].media[0].url}`);
      setDirection('older');
