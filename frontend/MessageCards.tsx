...
import { MEDIA_BASE_URL } from '@/utils/baseUrls';
      ...
      <div className={`flex ${isOwnMessage ? "justify-end items-end" : null} gap-2 relative`}>
        ...
        >
            ...
            <ReplyCard
              ...
              repliedMessage={
                msg.replyTo.messageType === "text"
                  ...
                  : msg.replyTo.media?.[0]?.url
                    ? `${MEDIA_BASE_URL}${msg.replyTo.media[0].url}`
                    : null
              }
              ...
              content={
                msg.messageType === "text"
                  ? msg.text
                  : msg.media?.[0]?.url
                    ? `${MEDIA_BASE_URL}${msg.media[0].url}`
                    : null
              }
            />
          )}
            ...
            msg.emojiType !== "standard" && (
              <>
                  ...
                  <div className="mt-2">
                    <ImageDisplay
                      img={msg.media
                        ...
                        .map(media => ({ img: `${MEDIA_BASE_URL}${media.url}` }))}
                      onImageClick={(index) =>
                        onImageClick(
                          index,
                          msg.media
                ...
                {msg.media?.length > 0 && msg.media.some(media => media.type === "audio") && (
                  <div className="mt-2">
                    <VoiceMessageCard
                      ...
                      audioUrl={`${MEDIA_BASE_URL}${msg.media.find(media => media.type === "audio")?.url}`}
                    />
                  </div>
                )}
                {msg.media?.length > 0 && msg.media.some(media => media.type === "video") && (
                  <div className="mt-2">
                    <VideoMessageCard
                      videoUrl={`${MEDIA_BASE_URL}${msg.media.find(media => media.type === "video")?.url}`}
                    />
                  </div>
                )}
                {msg.media?.length > 0 && msg.media.some(media => media.type === "file") && (
                  <div className="mt-2">
                    <ReplyCard
                      isOwnMessage={isOwnMessage}
                      contentType="file"
                      content={`${MEDIA_BASE_URL}${msg.media.find(media => media.type === "file")?.url}`}
                      themeIndex={themeIndex}
                    />
                  </div>
                )}
                ...
                {msg.img && (
                  <div className="mb-2">
                    <ImageDisplay img={[{ img: `${MEDIA_BASE_URL}${msg.img}` }]} onImageClick={() => onImageClick(0, [{ img: `${MEDIA_BASE_URL}${msg.img}` }])} />
                  </div>
                )}
              </>
            )}
          ...
          <button
            type="button"
            title="React"
            onClick={() => setIsReactionPickerOpen((v) => !v)}
