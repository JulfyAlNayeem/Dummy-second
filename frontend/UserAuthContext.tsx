...
const UserAuthProvider = ({ children }: { children: React.ReactNode }): JSX.Element => {
  ...
  const initializeSocket = useCallback(
    (currentUser) => {
      ...
      // In local dev, window.location.origin works because Vite proxies /socket.io
      // to the backend (see vite.config.js). In production with a split deployment
      // (frontend on Netlify, backend on Render), Netlify CANNOT proxy WebSocket
      // upgrades through its redirect rules — the socket must connect DIRECTLY to
      // the backend's own URL. Set VITE_SOCKET_URL in Netlify's environment
      // variables to your Render backend URL, e.g. https://your-backend.onrender.com
